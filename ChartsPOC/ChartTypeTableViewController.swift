...
class ChartTypeTableViewController: UITableViewController {
    ...
    override func tableView(_ tableView: UITableView, didSelectRowAt indexPath: IndexPath) {
        if indexPath.row == 0 {
            ...
            let axisStyle = AxisStyle(valueFormatter: {
                value in
                return "\(value) AM"
                
            })
            let nextVC = LineChartViewController(xAxisStyle:axisStyle, dataCalculationQueue: DispatchQueue.global(qos: .userInteractive)) {
                let values1 = (0..<80).map { (i) -> ChartDataEntry in
                    let val = Double(arc4random_uniform(80) + 3)
                    return ChartDataEntry(x: Double(i), y: val)
                }
